...
public final class UpdateChecker {
    ...
    private static final Component PREFIX = Component.text("[Blueprint]", NamedTextColor.BLUE);

    private final JavaPlugin plugin;
    private final Gson gson = new Gson();
    private final Supplier<List<Component>> messageSink;

    public UpdateChecker(JavaPlugin plugin, Supplier<List<Component>> messageSink) {
        this.plugin = plugin;
    ...
    public void checkUpdate() {
        try {
            ...
            if (changed) {
                List<Component> messages = messageSink.get();
                messages.clear();
                messages.add(PREFIX.append(Component.space())
                        .append(Component.text("New Update Available!", NamedTextColor.GOLD)));
                messages.add(PREFIX.append(Component.space())
                        .append(Component.text("Current v" + version + " >> Latest v" + latestVersion)));
                messages.addAll(getUpdateMessageDetails());
                messages.forEach(um -> Bukkit.getConsoleSender().sendMessage(um));
            }
        } catch (Exception ex) {
            Bukkit.getConsoleSender().sendMessage(Component.text("Could not check for updates!", NamedTextColor.RED));
    ...
    public List<Component> getUpdateMessageDetails() {
        SemanticLevel semanticLevel = GlobalConfig.getUpdateLoggingLevel();
        String version = plugin.getDescription().getVersion();
        String[] v = version.split("\\.");
        List<Component> updateMessages = new ArrayList<>();
        try {
            ...
            for (JsonElement element : jsonArray.asList()) {
                ...
                updateMessages.add(PREFIX.append(Component.space()).append(Component.text(update)));
            }
        } catch (Exception ex) {
            Bukkit.getConsoleSender().sendMessage(
                    Component.text("Could not check for detailed updates!", NamedTextColor.RED));
        }
